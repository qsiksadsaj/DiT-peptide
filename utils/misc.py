...
class Evaluator(object):
    ...
    def finalize_stats(self):
        rmsd_full_list, jsd_full_list = self.gather_results()

        stats = {}
        stats["RMSD"] = np.mean(rmsd_full_list) if rmsd_full_list else None
        stats["JSD"] = np.mean(jsd_full_list) if jsd_full_list else None
        return stats
    ...
    def gather_results(self):
        import torch.distributed as dist
        if not dist.is_initialized():
            return self.rmsd_list, self.jsd_list

        world_size = dist.get_world_size()

        rmsd_list = [None for _ in range(world_size)]
        dist.all_gather_object(rmsd_list, self.rmsd_list)
        rmsd_list = [x for rmsd in rmsd_list for x in rmsd]

        jsd_list = [None for _ in range(world_size)]
        dist.all_gather_object(jsd_list, self.jsd_list)
        jsd_list = [x for jsd in jsd_list for x in jsd]

        return rmsd_list, jsd_list
    ...
    def update(self, results, jsd):
        if "RMSD" in results:
            self.rmsd_list.append(results["RMSD"])
        self.jsd_list.append(jsd)
