...
class GaussianDiffusion:
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, y=None, mask=None, no_noise=False):
        """
        ...
        # === 不加噪模式（用于 overfit 检查） ===
        if no_noise:
            # print("❗ No noise mode, using data_start as prediction target")
            pred = denoise_fn(data_start, t, y, mask)  # t 不再重要
            diff = (data_start - pred) ** 2

            if mask is not None:
                mask = mask.to(diff.device).unsqueeze(1).float()  # (B, 1, N)
                diff = diff * mask
                loss_per_sample = diff.sum(dim=[1, 2])
                denom = mask.sum(dim=[1, 2]).clamp(min=1.0)
                losses = loss_per_sample / denom
            else:
        ...
        if self.loss_type == 'mse':
            eps_recon = denoise_fn(data_t, t, y, mask)
            assert data_t.shape == data_start.shape
            assert eps_recon.shape == torch.Size([B, D, N])

            # === mask-aware loss ===
            diff = (noise - eps_recon) ** 2    # (B, D, N)

            if mask is not None:
                # mask shape: (B, N) → (B, 1, N)
                mask = mask.to(diff.device).unsqueeze(1).float()          # (B, 1, N)
                diff = diff * mask                        # zero out padding parts

                loss_per_sample = diff.sum(dim=[1, 2])    # (B,)
                denom = mask.sum(dim=[1, 2]).clamp(min=1.0)  # avoid divide by zero
                losses = loss_per_sample / denom
            else:
                losses = diff.mean(dim=[1, 2])            # fallback
...
class Model(nn.Module):
    ...
    def _denoise(self, data, t, y, mask=None): 
        ...
        out = self.model(data, t, y, mask)  # 调用Dit3D模型 输入噪声样本 x 时间步 t 条件 y 输出预测的噪声

        assert out.shape == torch.Size([B, D, N])
        if torch.isnan(out).any():
            print("❗NaN detected in model output (eps recon)")
        return out
    ...
    def get_loss_iter(self, data, mask, noises=None, y=None):
        ...
        # noises = torch.randn_like(data)  # 改成对于每个 batch 动态生成 Noise
        # # noises = torch.zeros_like(data)

        noises = torch.zeros_like(data)
        if (t != 0).any():
            noises[t != 0] = torch.randn((t != 0).sum(), D, N, device=data.device)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            # loss = model.get_loss_iter(x, noises_batch, y).mean()  # 前向传播并计算返回 loss
            # peptide不定长 预先生成noise_batch 没有意义 
            loss = model.get_loss_iter(x, y=y, mask=mask).mean()
...
def main():
    ...
    ''' workaround '''
    train_dataset, _ = get_dataset(opt.dataroot, opt.npoints, opt.category)
    noises_init = torch.randn(len(train_dataset), opt.npoints, opt.nc)  # 噪声初始化 shape [len(train_dataset), N_sample, 3] 相当于为每个样本初始化一个噪声 作为 diffusion 起点
...
def parse_args():
    ...
    # Data params
    parser.add_argument('--dataroot', default='/root/DiT-3D/data/peptides_only_ligand_raw')  # 输入的数据文件夹
    parser.add_argument('--category', default='peptide')
    parser.add_argument('--num_classes', type=int, default=55)  # 没啥用目前
    ...
    parser.add_argument('--npoints', default=128)  # xj
    parser.add_argument("--voxel_size", type=int, choices=[16, 32, 64, 128, 256], default=32)  # 分辨率 太大了会爆显存
    ...
    parser.add_argument('--grad_clip', type=float, default=1.0, help='weight decay for EBM')  # 梯度裁剪 得开
    ...
    '''eval'''
    parser.add_argument('--saveIter', default=500, type=int, help='unit: epoch')  # 每多少轮保存一下模型
