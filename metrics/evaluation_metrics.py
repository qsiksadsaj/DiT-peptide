...
def _pairwise_EMD_CD_peptide(sample_pcs_list, ref_pcs_list, batch_size, accelerated_cd=True):
    """
    sample_pcs_list: list of tensors, each (N_i, 3)
    ref_pcs_list: list of tensors, each (M_j, 3)
    """
    N_sample = len(sample_pcs_list)
    N_ref = len(ref_pcs_list)
    all_cd = []
    all_emd = []

    for i in range(N_sample):
        sample_batch = sample_pcs_list[i]         # (N_i, 3)

        cd_lst = []
        emd_lst = []

        for j in range(0, N_ref, batch_size):
            ref_batch_list = ref_pcs_list[j : j + batch_size]

            # pad ref_batch into a single batch tensor
            ref_batch = pad_pointcloud_batch(ref_batch_list)   # (B, max_M, 3)

            B, max_M, _ = ref_batch.shape

            # expand sample_batch → (B, N_i, 3)
            sample_batch_exp = sample_batch.unsqueeze(0).expand(B, -1, -1).contiguous()

            dl, dr, _, _ = cham3D(
                sample_batch_exp.cuda(),
                ref_batch.cuda()
            )
            cd = (dl.mean(dim=1) + dr.mean(dim=1)).view(1, -1).detach().cpu()
            cd_lst.append(cd)
        ...
        cd_lst = torch.cat(cd_lst, dim=1)   # [1, N_ref]
        emd_lst = torch.cat(emd_lst, dim=1)
...
def pad_pointcloud_batch(tensor_list, padding_value=0.0):
    """
    将 list of (N, 3) → pad → (B, max_N, 3)
    """
...
def compute_rmsd_peptide(sample_pcs, ref_pcs):
    """
    Compute RMSD between generated peptide and ground truth peptide.
    
    Args:
        sample_pcs (torch.Tensor): [1, N, 3]
        ref_pcs (torch.Tensor): [1, N, 3]
        
    Returns:
        dict: { "RMSD": float }
    """
    # 去掉 batch 维
    gen_np = sample_pcs.squeeze(0).cpu().numpy()   # [N, 3]
    ref_np = ref_pcs.squeeze(0).cpu().numpy()      # [N, 3]
    
    # 坐标差
    diff = gen_np - ref_np                        # [N, 3]
    sq = (diff ** 2).sum(axis=1)                  # [N]
    # rmsd = np.sqrt(sq).mean()
    rmsd = np.sqrt(sq.mean())
    
    return {"RMSD": float(rmsd)}

def compute_all_metrics_peptide(sample_pcs, ref_pcs, sample_masks, ref_masks, batch_size):
    """
    sample_pcs: (B, N, 3)
    ref_pcs: (B, N, 3)
    sample_masks: (B, N)
    ...
    for i in range(sample_pcs.shape[0]):
        s_mask = sample_masks[i] > 0    # (N,)
        r_mask = ref_masks[i] > 0

        s_pts = sample_pcs[i][s_mask]    # (valid_N, 3)
        r_pts = ref_pcs[i][r_mask]

        # 如果全是 padding，避免空 tensor 报错
        if s_pts.shape[0] == 0:
            s_pts = torch.zeros((1, 3), dtype=sample_pcs.dtype, device=sample_pcs.device)
        if r_pts.shape[0] == 0:
            r_pts = torch.zeros((1, 3), dtype=ref_pcs.dtype, device=ref_pcs.device)

        sample_pcs_clean.append(s_pts)
        ref_pcs_clean.append(r_pts)

    # === 把变长 list → batch (List[N_i, 3]) ===
    # _pairwise_EMD_CD_ 接受 list of Tensors
    results = {}

    # Pairwise distance
    M_rs_cd, M_rs_emd = _pairwise_EMD_CD_peptide(
        ref_pcs_clean,
        sample_pcs_clean,
        batch_size
    )
    ...
    M_rr_cd, M_rr_emd = _pairwise_EMD_CD_peptide(
        ref_pcs_clean,
        ref_pcs_clean,
        batch_size
    )
    M_ss_cd, M_ss_emd = _pairwise_EMD_CD_peptide(
        sample_pcs_clean,
        sample_pcs_clean,
        batch_size
    )
