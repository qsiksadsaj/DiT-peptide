...
from datasets.peptide_data_label import PeptidePointClouds, peptide_collate_fn

from copy import deepcopy
from collections import OrderedDict

from models.dit3d_labels import DiT3D_models
from models.dit3d_window_attn import DiT3D_models_WindAttn

from tensorboardX import SummaryWriter
from modules.loss import label_distribution_loss, spatial_affinity_loss
...
class GaussianDiffusion:
    ...
    def p_losses(self, denoise_fn, data_start, labels, t, noise=None, y=None, mask=None):
        """
        Compute coordinate loss + label loss for diffusion

        Args:
            denoise_fn: the model
            data_start: [B, 3, N] - original clean point cloud
            labels:     [B, N]    - ground truth labels for each point
            t:          [B,]
            noise:      [B, 3, N] or None
            y:          [B,] optional class condition
            mask:       [B, N]    mask indicating valid points

        Returns:
            loss_coord: scalar tensor
            loss_label: scalar tensor
        """
        ...
        if self.loss_type == 'mse':
            pred_noise, pred_logits = denoise_fn(data_t, t, y, mask, labels)
            assert data_t.shape == data_start.shape
            assert pred_noise.shape == torch.Size([B, D, N])

            # === mask-aware loss ===
            diff = (noise - pred_noise) ** 2    # (B, D, N)

            if mask is not None:
                # mask shape: (B, N) → (B, 1, N)
                mask3d = mask.to(diff.device).unsqueeze(1).float()          # (B, 1, N)
                diff = diff * mask3d                        # zero out padding parts
                loss_per_sample = diff.sum(dim=[1, 2])    # (B,)
                denom = mask3d.sum(dim=[1, 2]).clamp(min=1.0)  # avoid divide by zero
                loss_coord = loss_per_sample / denom
            else:
                loss_coord = diff.mean(dim=[1, 2])            # fallback

            # === label losses ===
            if pred_logits is None:
                loss_label_dist = torch.tensor(0.0, device=data_start.device)
                loss_affinity = torch.tensor(0.0, device=data_start.device)
            else:
                loss_label_dist = label_distribution_loss(pred_logits, labels, mask)
                loss_affinity = spatial_affinity_loss(pred_logits, pred_noise, mask)
...
class Model(nn.Module):
    ...
    def prior_kl(self, x0):  # 计算先验分布KL散度 用于无监督训练下的likelihood估计
    ...
    def _denoise(self, data, t, y, mask=None, labels=None): 
        """
        Denoise function for diffusion model.
        ...
        Returns:
            pred_noise: [B, 3, N]
            pred_logits: [B, num_classes, N]
        """
        B, D,N= data.shape
        assert data.dtype == torch.float
        assert t.shape == torch.Size([B]) and t.dtype == torch.int64

        pred_noise, pred_logits  = self.model(data, t, y, mask)  # 调用Dit3D模型 输入噪声样本 x 时间步 t 条件 y 输出预测的噪声

        # shape checks
        assert pred_noise.shape == (B, D, N), f"pred_noise shape mismatch: {pred_noise.shape}"
        if pred_logits is not None:
            assert pred_logits.shape[0] == B, f"pred_logits batch mismatch: {pred_logits.shape}"
            assert pred_logits.shape[2] == N, f"pred_logits length mismatch: {pred_logits.shape}"

        return pred_noise, pred_logits

    # 对一个batch 计算训练 loss
    def get_loss_iter(self, data, labels, mask, noises=None, y=None):
        B, D, N = data.shape                           # [16, 3, 2048]
        t = torch.randint(0, self.diffusion.num_timesteps, size=(B,), device=data.device)  # 为每个样本采样一个时间步t
        
        # if noises is not None:
        #     noises[t!=0] = torch.randn((t!=0).sum(), *noises.shape[1:]).to(noises)  # 给所有t≠0的样本重新采样噪声 噪声是随机采样的 但是注入噪声的时候注入程度和 t 时间步有关

        losses = self.diffusion.p_losses(
            denoise_fn=self._denoise, data_start=data, labels=labels, t=t, noise=noises, y=y, mask=mask)
        assert losses.shape == t.shape == torch.Size([B])
        return losses

    def gen_samples(self, shape, device, y, noise_fn=torch.randn,
                    clip_denoised=True, 
                    keep_running=False):  # 从纯噪声开始反推
    ...
    def gen_sample_traj(self, shape, device, y, freq, noise_fn=torch.randn,
                    clip_denoised=True,keep_running=False):  # 记录每一步的中间结果
...
def get_dataloader(opt, train_dataset, test_dataset=None):
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=opt.bs,sampler=train_sampler,
                                                   shuffle=train_sampler is None, num_workers=int(opt.workers), drop_last=True, collate_fn=peptide_collate_fn)

    if test_dataset is not None:
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=opt.bs,sampler=test_sampler,
                                                   shuffle=False, num_workers=int(opt.workers), drop_last=False, collate_fn=peptide_collate_fn)
    else:
...
def train(gpu, opt, output_dir):
    ...
    if opt.distribution_type == 'multi':  # 如果是分布式多GPU  就只有 rank=0 负责写日志和保存文件 避免多进程同时写入同一个文件导致崩溃
        should_diag = gpu==0
    else:
        should_diag = True
    if should_diag:  # 创建输出目录
        outf_syn, = setup_output_subdirs(output_dir, 'syn')

    if opt.distribution_type == 'multi':  # 分布式多GPU训练
    ...
    betas = get_betas(opt.schedule_type, opt.beta_start, opt.beta_end, opt.time_num)  # 生成diffusion每一步的噪声强度
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            '''
            data.keys(['idx', 'train_points', 'test_points', 'lengths', 'train_masks', 'test_masks', 'mean', 'std', 'cate_idx', 'sid', 'mid'])
            idx: 当前样本的索引
            train_points: 训练集点云数据 [B, N, 3]
            test_points: 测试集点云数据 [B, N, 3]
            lengths: 训练集点云数据长度 [B,]   记录每个样本的点云长度
            train_masks: 训练集点云数据掩码 [B, N]  记录每个样本的点云是否有效 
            test_masks: 测试集点云数据掩码 [B, N]  记录每个样本的点云是否有效
            mean: 训练集点云数据均值 [B, 1, 3]
            std: 训练集点云数据标准差 [B, 1, 3]
            cate_idx: 当前样本的类别索引 [B,]  因为都是 peptide 数据集 所以 cate_idx 是 0
            sid: 当前样本的ShapeNet ID [B,] 都是 peptide list of str 都是 peptide
            mid: 当前样本的ModelNet ID [B,] ShapeNet每个 shape 的唯一 id 如train/7qs9_e
            '''
            x = data['train_points'].transpose(1,2)
            y = data['cate_idx']
            mask = data['train_masks']  # [B, N] 记录每个样本的点云是否有效 加的 mask 部分
            labels_gt = data["train_labels"]
            '''
            train diffusion
            '''
            ...
            # === generate dynamic noise # NEW
            noise = torch.randn_like(x.transpose(1,2))    # [B, N, 3]
            noise[~mask] = 0.0
            noise = noise.transpose(1, 2)                 # [B, 3, N]

            losses = model.get_loss_iter(
                x, labels=labels_gt, y=y, mask=mask
            )
            # loss = loss_coord + opt.label_loss_weight * loss_label
            loss = losses.mean() 
            optimizer.zero_grad()
            loss.backward()
            ...
            if not opt.debug:
                ...
                if opt.use_tb:
                    # tb_writer.add_scalar('train_loss_coord', loss_coord.item(), global_step)
                    # tb_writer.add_scalar('train_loss_label', loss_label.item(), global_step)
                    tb_writer.add_scalar('train_total_loss', loss.item(), global_step)
...
def main():
    ...
    # Use random port to avoid collision between parallel jobs 分布式训练
    if opt.world_size == 1:
...
def parse_args():
    ...
    parser.add_argument('--experiment_name', type=str, default='dit3d_label', help='experiment name (used for checkpointing and logging)')

    # Data params
    parser.add_argument('--dataroot', default='/root/DiT-3D/data/peptides_only_ligand_label')
