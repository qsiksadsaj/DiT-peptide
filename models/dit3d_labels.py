...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_size = hidden_size  # Transformer 的隐藏层维度
        ...
        self.final_layer = FinalLayer(hidden_size, patch_size, self.out_channels) # 最终输出层
        self.label_layer = FinalLayer(hidden_size, patch_size, hidden_size)  # label提取层
    ...
    def unpatchify_voxels(self, x0, c_shape=None):
        """
    ...
    def forward(self, x, t, y, mask=None):
        ...
        # 点坐标回归
        x_point = self.final_layer(x, c)  # 将输出还原为patch格式            
        x_point = self.unpatchify_voxels(x_point)  # 将patch恢复为完整的体素体                   
        # Devoxelization
        pred_noise = F.trilinear_devoxelize(x_point, voxel_coords, self.input_size, self.training)

        # 点分类
        x_label = self.label_layer(x, c)  # 将输出还原为patch格式  
        x_point_fea = self.unpatchify_voxels(x_label, self.hidden_size)  # 将patch恢复为完整的体素体                   
        # Devoxelization
        pred_noise_fea = F.trilinear_devoxelize(x_point_fea, voxel_coords, self.input_size, self.training)
        pred_logits = self.classifier(pred_noise_fea) 
