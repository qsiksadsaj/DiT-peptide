...
class DiT(nn.Module):
    ...
    # xj
    def unpatchify_voxels(self, x, grid_size, patch_size, channel_dim):
        """
        x: [B, N_patch, patch_dim]
        grid_size: (x, y, z)
        patch_size: int
        channel_dim: int (e.g. 64)
        """
        B, N_patch, patch_dim = x.shape
        x_count, y_count, z_count = grid_size
        p = patch_size
        c = channel_dim

        assert patch_dim == c * p ** 3, f"Mismatch: got patch_dim={patch_dim}, expected {c}*{p}^3"

        x = x.view(B, x_count, y_count, z_count, p, p, p, c)
        x = x.permute(0, 7, 1, 4, 2, 5, 3, 6).contiguous()  # [B, C, D, H, W]
        x = x.view(B, c, x_count * p, y_count * p, z_count * p)
        return x

    # xj
    def forward(self, x, t, y, mask=None, skip_network=True):
        """
        Forward pass of DiT-3D.
        Args:
            x: [B, C=3, N] point cloud
            t: [B] timestep
            y: [B] class label
            mask: [B, N] optional mask for valid points
            skip_network: bool, if True, bypass transformer layers for testing voxel⟷point⟷voxel consistency
        """
        if mask is not None:
        ...
        # # 如果是高斯 voxel   走 feature 处理
        # features = features - features.mean(dim=2, keepdim=True)
        # features = features / (features.norm(dim=1, keepdim=True).max(dim=2, keepdim=True).values + 1e-6)

        x_voxel, voxel_coords = self.voxelization(features, coords)  # → [B, C, D, H, W], [B, C, N]

        B, C, D, H, W = x_voxel.shape
        p = self.patch_size
        x_count, y_count, z_count = D // p, H // p, W // p
        patch_dim = C * (p ** 3)

        # print(f"[Debug] voxel shape: {x_voxel.shape}")
        # print(f"[Debug] voxel_coords shape: {voxel_coords.shape}")
        # print(f"[Debug] voxel_coords range: min={voxel_coords.min().item():.4f}, max={voxel_coords.max().item():.4f}")
        # print(f"[Debug] voxel_coords unique: {torch.unique(voxel_coords).shape[0]} values")

        # === Patchify ===
        x_patch = x_voxel.view(B, C, x_count, p, y_count, p, z_count, p)
        x_patch = x_patch.permute(0, 2, 4, 6, 3, 5, 7, 1).contiguous()  # → [B, x, y, z, p, p, p, C]
        x_patch = x_patch.view(B, x_count * y_count * z_count, patch_dim)  # [B, N_patch, patch_dim]

        # === Skip Transformer ===
        if skip_network:
        #     print(f"[Debug] Skipping transformer blocks...")
            x_voxel_recon = self.unpatchify_voxels(
                x_patch,
                grid_size=(x_count, y_count, z_count),
                patch_size=p,
                channel_dim=C
            )
            # print(f"[Debug] unpatchified voxel shape: {x_voxel_recon.shape}")
            # print(f"[Debug] unpatchified voxel absmax: {x_voxel_recon.abs().max().item():.6f}")
            x_out = F.trilinear_devoxelize(x_voxel_recon, voxel_coords, self.input_size, self.training)
            return x_out

        # === Embed tokens & pos ===
        x_patch = self.x_embedder(x_patch)  # linear embedding
        x_patch = x_patch + self.pos_embed

        # === Time & Label Embedding ===
        t_embed = self.t_embedder(t)
        y_embed = self.y_embedder(y, self.training)
        c = t_embed + y_embed  # conditional embedding

        for block in self.blocks:
            x_patch = block(x_patch, c)

        x_patch = self.final_layer(x_patch, c)

        # === Unpatchify & Devoxelize ===
        x_voxel_recon = self.unpatchify_voxels(
            x_patch,
            grid_size=(x_count, y_count, z_count),
            patch_size=p,
